...
class $modify(EchoPlayLayer, PlayLayer) {
    ...
    bool init(GJGameLevel* level, bool useReplay, bool dontCreateObjects) {
        ...
        if (GhostPlay.hasGhost()) {
            ...
            if (auto* objectLayer = this->m_objectLayer) {
    ...
    void update(float dt) {
        ...
        if (!this->m_isPaused && !this->m_hasCompletedLevel) {
            m_fields->m_gameTime += dt;
            
            // Начало игры (после старта)
            if (!m_fields->m_hasStarted && this->m_player1 && 
                this->m_player1->getPositionX() > 0) {
        ...
        // Записываем кадр
        if (GhostRec.isRecording() && this->m_player1 && m_fields->m_hasStarted) {
            GhostRec.recordFrame(this->m_player1, false);
            
            // Записываем второго игрока если dual mode
            if (this->m_player2 && this->m_isDualMode) {
                GhostRec.recordFrame(this->m_player2, true);
            }
            
            // Обновляем процент
            int currentPercent = static_cast<int>(this->m_player1->getPositionX() / 
                                                   this->m_levelLength * 100);
    ...
    void resetLevel() {
        ...
        if (this->m_level) {
            GhostRec.startRecording(this->m_level);
        }
    ...
    void levelComplete() {
        ...
        if (GhostRec.shouldSaveRecording()) {
            ...
            std::string levelID = std::to_string(this->m_level->m_levelID.value());
            GhostPlay.loadGhostForLevel(levelID);
        }
